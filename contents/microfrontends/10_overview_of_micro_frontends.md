...
これら3つの内、検索結果のサーバーサイドレンダリングが起因してレスポンスタイムが遅延した場合、次の手段を検討する。
...
1番は、アプリケーションの複雑さを増してしまう可能性がある。
2番は、応急手当であり、抜本的な解決ではない。
3番は、札束でマシンを叩くだけで解決するが、無駄な出費が多い。

この内、筆者としては1番と3番を考えたい。
1番で保守性を損なわない解決策があれば、それを採用する。
たとえば、機能の並列処理化や、非同期処理を検討などだ。
もしなければ、それは必要なマシン増強だと判断し、3番を選択する。
しかし、個別機能のためだけにマシン増強していては、他機能のマシンリソース活用が不十分になってしまう。
